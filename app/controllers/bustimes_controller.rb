class BustimesController < ApplicationController
  ...
  def buspredictions
    set_recent_stops_cookie(params[:id], params[:rtnm], params[:dir],
                            params[:stopid], params[:stpnm])
    @buspredictions = Bustime.new.buspredictions(params[:id], params[:dir],
                                                 params[:stopid])
  end

  def set_recent_stops_cookie(id, rtnm, dir, stopid, stpnm)
    cookies[:recent_stops] = {
      value: ActiveSupport::JSON.encode(
        append_cookie_value(id, rtnm, dir, stopid, stpnm)),
  ...
  def append_cookie_value(id, rtnm, dir, stopid, stpnm)
    ...
    recent_stops.pop if recent_stops.count > 5
